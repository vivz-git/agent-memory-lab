...
from src.agent.core import RegAgent, MockLLMClient, Demonstration
...
def main():
    Path("evaluation/pilot").mkdir(parents=True, exist_ok=True)
    ...
    config = {
        ...
        "seed": seed,
        "llm_client": "MockLLMClient(mode='demonstration_mimic')",
    }
    
    with open("evaluation/pilot/config.json", "w") as f:
    ...
    with open("evaluation/pilot/results.json", "w") as f:
        json.dump(results, f, indent=2)
        
    print("\nPilot complete. Results written to evaluation/pilot/")
